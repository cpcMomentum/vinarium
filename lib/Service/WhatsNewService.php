...
/**
 * „Was ist neu?"-Fenster (Baustein 0b, #284).
 *
 * Uebernommen aus dem Pilot-Einbau in RechnungsWerk (cpcMomentum/rechnungswerk#308,
 * ausgeliefert in v0.5.3). Die Inhalte liegen als `whatsnew/whatsnew.json` im
 * App-Paket — kein Nachladen von fremden Servern, die App telefoniert nicht nach
 * Hause. Gemerkt wird pro Nutzer die zuletzt gesehene App-Version.
 *
 * Verhalten (Konzept v1.1, am Prototyp abgenommen):
 ...
 *
 * Abweichung von der Vorlage: Vinarium ist eine private Weinkeller-App und traegt
 * keine WerkPlus-Funktionen. Das `plus`-Feld der Vorlage (Badge + Link auf
 * werkwolke.de) entfaellt deshalb hier vollstaendig.
 */
class WhatsNewService {
	 ...
	 * gesetzt von {@see \OCA\Vinarium\Migration\WhatsNewBootstrap}.
	 ...
	 * heisst: kein Fenster.
	 *
	 * @return array{version: string, entries: list<array{title: string, text: string, icon: string, where: string, adminOnly: bool}>}
	 */
	...
	private function readCatalogue(): array {
		...
		foreach ($decoded as $version => $entries) {
			...
			// x.y.z-Schema kommt das nicht vor, aber ein stilles continue faellt
			// hier niemandem auf — also lieber sauber auf String ziehen.
	 ...
	 * @return list<array{title: string, text: string, icon: string, where: string, adminOnly: bool}>
	 */
