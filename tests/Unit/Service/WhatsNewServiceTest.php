...
/**
 * Anzeige-Logik des „Was ist neu?"-Fensters (#284).
 *
...
class WhatsNewServiceTest extends TestCase {
	...
	private const BEKANNTE_SYMBOLE = [
		'bottle-wine', 'camera', 'chart-bar', 'cog', 'folder',
		'glass-wine', 'grid', 'magnify', 'star', 'translate',
	];
	...
	protected function setUp(): void {
		parent::setUp();
		$this->appDir = sys_get_temp_dir() . '/vin-whatsnew-' . bin2hex(random_bytes(6));
	...
	private function catalogueFixture(): array {
		return [
			...
			'0.5.4' => [
				[
					'title' => ['de' => 'Etikett mit zwei Seiten', 'en' => 'Label with two sides'],
					'text' => ['de' => 'Vorder- und Rueckseite', 'en' => 'Front and back'],
					'icon' => 'camera',
					'where' => ['de' => 'Bestand', 'en' => 'Inventory'],
					'adminOnly' => false,
				],
				[
					'title' => ['de' => 'Regal-Reihenfolge', 'en' => 'Shelf order'],
					'text' => ['de' => 'Per Tastatur verschiebbar', 'en' => 'Movable by keyboard'],
					'icon' => 'grid',
					'where' => ['de' => 'Verwaltung', 'en' => 'Administration'],
					'adminOnly' => true,
				],
			],
		];
	...
	public function testBestandsinstallationSiehtDasEinfuehrungsFenster(): void {
		...
		$this->writeCatalogue($this->catalogueFixture());
		$written = [];
		$service = $this->buildService('0.5.4', [], $written, 'de', '0.5.4');
	...
	public function testNotizGiltNurFuerDasEinfuehrendeRelease(): void {
		// Die Notiz stammt von 0.5.4, installiert ist inzwischen 0.5.5. Ein
		// Konto, das erst jetzt dazukommt, ist wirklich neu.
		$this->writeCatalogue(['0.5.4' => $this->catalogueFixture()['0.5.4'], '0.5.5' => [[
			'title' => ['de' => 'Spaeter', 'en' => 'Later'],
			'text' => ['de' => 'Text', 'en' => 'Text'],
		]]]);
		$written = [];
		$service = $this->buildService('0.5.5', [], $written, 'de', '0.5.4');

		$result = $service->getPending('alice');

		self::assertSame([], $result['entries'], 'Spaetere Neuzugaenge werden nicht begruesst');
		self::assertSame('0.5.5', $written['alice/' . WhatsNewService::KEY_LAST_SEEN] ?? null);
	}

	public function testOhneNotizBleibtEsBeimErstkontakt(): void {
		$this->writeCatalogue($this->catalogueFixture());
		$written = [];
		$service = $this->buildService('0.5.4', [], $written, 'de', '');
	...
	public function testNeueVersionLiefertDerenEintraege(): void {
		...
		self::assertSame('0.5.4', $result['version']);
		self::assertCount(2, $result['entries']);
		self::assertSame('Etikett mit zwei Seiten', $result['entries'][0]['title']);
		self::assertSame('Vorder- und Rueckseite', $result['entries'][0]['text']);
		self::assertSame('camera', $result['entries'][0]['icon']);
		self::assertSame('Bestand', $result['entries'][0]['where']);
		self::assertFalse($result['entries'][0]['adminOnly']);
		self::assertTrue($result['entries'][1]['adminOnly'], 'Die Verwaltung ist adminpflichtig');
		self::assertArrayNotHasKey(
			'alice/' . WhatsNewService::KEY_LAST_SEEN,
			$written,
	...
	public function testVersionssprungKumuliertNicht(): void {
		$this->writeCatalogue($this->catalogueFixture());
		$written = [];
		$service = $this->buildService('0.5.4', ['alice/' . WhatsNewService::KEY_LAST_SEEN => '0.4.0'], $written);
	...
	public function testNichtsNeuesSchreibtDieMarkeStillFort(): void {
		$this->writeCatalogue($this->catalogueFixture());
		$written = [];
		$service = $this->buildService('0.5.5', ['alice/' . WhatsNewService::KEY_LAST_SEEN => '0.5.4'], $written);
	...
	public function testEintraegeAusDerZukunftBleibenAus(): void {
		$this->writeCatalogue($this->catalogueFixture());
		$written = [];
		// Installiert ist 0.5.3, die Datei kennt schon 0.5.4.
		$service = $this->buildService('0.5.3', ['alice/' . WhatsNewService::KEY_LAST_SEEN => '0.5.0'], $written);
	...
	public function testEnglischeNutzerBekommenDenEnglischenText(): void {
		$this->writeCatalogue($this->catalogueFixture());
		$written = [];
		$service = $this->buildService('0.5.4', ['alice/' . WhatsNewService::KEY_LAST_SEEN => '0.5.3'], $written, 'en');

		$result = $service->getPending('alice');

		self::assertSame('Label with two sides', $result['entries'][0]['title']);
	}
	...
	public function testFundortFolgtDerSprache(): void {
		...
		self::assertSame('Inventory', $result['entries'][0]['where']);
	}

	public function testEintragOhneSymbolUndFundortBleibtGueltig(): void {
		$this->writeCatalogue(['0.5.4' => [[
	...
	public function testRegionalcodeFaelltAufDieBasisspracheZurueck(): void {
		$this->writeCatalogue($this->catalogueFixture());
		$written = [];
		$service = $this->buildService('0.5.4', ['alice/' . WhatsNewService::KEY_LAST_SEEN => '0.5.3'], $written, 'de_DE');
	...
	public function testUnbekannteSpracheFaelltAufEnglischZurueck(): void {
		$this->writeCatalogue($this->catalogueFixture());
		$written = [];
		$service = $this->buildService('0.5.4', ['alice/' . WhatsNewService::KEY_LAST_SEEN => '0.5.3'], $written, 'fr');
	...
	public function testMarkenZweierNutzerBleibenGetrennt(): void {
		$this->writeCatalogue($this->catalogueFixture());
		$written = [];
		// Alice hat das Fenster schon gesehen, Bob steht noch auf 0.5.3.
		$service = $this->buildService('0.5.4', [
			'alice/' . WhatsNewService::KEY_LAST_SEEN => '0.5.4',
			'bob/' . WhatsNewService::KEY_LAST_SEEN => '0.5.3',
		], $written);
		...
		self::assertSame('0.5.4', $written['bob/' . WhatsNewService::KEY_LAST_SEEN]);
	}
	...
	public function testDieAusgelieferteDateiIstGueltig(): void {
		...
		foreach ($catalogue as $version => $entries) {
			...
			foreach ($entries as $entry) {
				...
				// Vinarium traegt keine WerkPlus-Funktionen; das Feld der Vorlage
				// waere hier ein totes Gewicht, das niemand mehr pflegt.
	...
	public function testAppIdBleibtDerAblageortDerMarke(): void {
		...
		self::assertSame('vinarium', Application::APP_ID);
	}
}
