...
| Lokale Entwicklung | Nextcloud im Docker (OrbStack), Mount nach `/var/www/html/custom_apps/vinarium` |
...
| `lint_backend` | `docker exec -t nextcloud-dev php -l /var/www/html/custom_apps/vinarium/lib/**/*.php` |
| `format_frontend` | `npx prettier --write src/` |
| `format_backend` | `N/A` |
| `typecheck` | `N/A` |
| `test_frontend` | `npm run test` |
| `test_backend` | `docker exec -t nextcloud-dev php /var/www/html/custom_apps/vinarium/vendor/bin/phpunit -c /var/www/html/custom_apps/vinarium/tests/phpunit.xml` |
...
> **Hinweis:** `vinarium` wird bei `/adopt` durch den tatsaechlichen App-Namen (aus `appinfo/info.xml`) ersetzt. PHP-Lint laeuft im Docker-Container weil PHP auf macOS typisch nicht installiert ist.
...
| Assets-Pfad: `js/vinarium-main.js` | NC laedt JS aus `js/`, nicht aus `dist/` |
...
| Hash-Routing im Vue Router | NC-Apps laufen unter `/apps/vinarium/` ohne History-API |
...
| Signatur | `openssl dgst -sha512 -sign ~/.nextcloud/certificates/vinarium.key` |
...
| Regel | Details |
|-------|---------|
| Build Output pruefen | `npm run build` muss `js/vinarium-main.js` erzeugen |
...
| Tabellennamen | Prefix mit App-ID: `vinarium_<tablename>` |
