...
| Lokale Entwicklung | Nextcloud im Docker (OrbStack), Mount nach `/var/www/html/custom_apps/<appname>` |
...
| `lint_backend` | `docker exec -t nextcloud-dev php -l /var/www/html/custom_apps/<appname>/lib/**/*.php` |
| `format_frontend` | `npx prettier --write src/` |
| `format_backend` | `N/A` |
| `typecheck` | `N/A` |
| `test_frontend` | `npm run test` |
| `test_backend` | `docker exec -t nextcloud-dev php /var/www/html/custom_apps/<appname>/vendor/bin/phpunit -c /var/www/html/custom_apps/<appname>/tests/phpunit.xml` |
...
> **Hinweis:** `<appname>` wird bei `/adopt` durch den tatsaechlichen App-Namen (aus `appinfo/info.xml`) ersetzt. PHP-Lint laeuft im Docker-Container weil PHP auf macOS typisch nicht installiert ist.
...
| Assets-Pfad: `js/<appname>-main.js` | NC laedt JS aus `js/`, nicht aus `dist/` |
...
| Hash-Routing im Vue Router | NC-Apps laufen unter `/apps/<appname>/` ohne History-API |
...
| Signatur | `openssl dgst -sha512 -sign ~/.nextcloud/certificates/<appname>.key` |
...
| Regel | Details |
|-------|---------|
| Build Output pruefen | `npm run build` muss `js/<appname>-main.js` erzeugen |
...
| Tabellennamen | Prefix mit App-ID: `<appname>_<tablename>` |
